...
import { MoreVerticalIcon, PackageOpenIcon, PlusIcon, TrashIcon } from "lucide-react";
import Link from "next/link";
import { SearchIcon, Loader2Icon, AlertTriangleIcon } from "lucide-react";
  ...
  return(
    <Empty className="border border-dashed bg-white">
      <EmptyHeader>
        ...
        {Boolean(message) &&(<EmptyDescription>
          {message}
        </EmptyDescription>)}
        {!!onNew &&(<EmptyContent>
    ...
    <Link href={href} prefetch>
      <Card className={cn("p-4 shadow-none hover:shadow cursor-pointer",isRemoving && "opacity-50 cursor-not-allowed",className)}>
        <CardContent className="flex flex-row items-center justify-between p-0">
          ...
          {(actions || onRemove) &&(
            <div className="felx gap-x-4 items-center">
              {actions}
              {onRemove && (
                <DropdownMenu>
