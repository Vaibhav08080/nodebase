...
export const EditorNameInput = ({ workflowId }: { workflowId: string }) => {
  ...
  const UpdateWorkflows = useUpdateWorkflowName();
  ...
  const handleSave = async () => {
    ...
    try {
      await UpdateWorkflows.mutateAsync({ id: workflowId, name: name });
    } catch (error) {
      setName(workflow.name);
    } finally {
      setIsEditing(false);
  ...
  if (isEditing) {
    return (
      <Input
      disabled={UpdateWorkflows.isPending}
      ref={inputRef}
        value={name}
    ...
    <BreadcrumbItem onClick={()=>setIsEditing(true)} className="cursor-pointer hover:text-foreground transition-colors">
      {isEditing ? (
        <Input
          value={name}
          onChange={(e) => setName(e.target.value)}
          onBlur={() => UpdateWorkflows.mutate({ id: workflowId, name: name })}
        />
      ) : (
